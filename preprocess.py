...
def init_argparse():
    ...
    parser.add_argument('ZIPFILE', nargs=1, action='store', type=Path, metavar='takeout-20230325T162834Z-001.zip', help='Google Timeline Takeout zip file')
    parser.add_argument('STARTDATE', nargs=1, action='store', metavar='DD/MM/YYYY', help='Start date')
    parser.add_argument('-u', '--until', action='store', default=[datetime.today().strftime('%d/%m/%Y')], dest='ENDDATE', metavar='DD/MM/YYYY', required=False, help='End date (default: today)')
    parser.add_argument('-e', '--error', action='store', default=1, dest='ERROR', required=False, type=float, help='Error distance (in km) to add to locations')
...
if __name__ == '__main__':
    ...
    startdate, enddate = [datetime.strptime(d[0], '%d/%m/%Y') for d in (args.STARTDATE, args.ENDDATE)]
    if enddate < startdate:
        exit(f"Start date ({args.STARTDATE}) needs to be before end date ({args.ENDDATE})")
